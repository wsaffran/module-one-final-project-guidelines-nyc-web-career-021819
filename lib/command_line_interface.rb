...
def what_next_selections
  ...
  if answer != "1" && answer != "2" && answer != "3" && answer != "4"
    ...
    puts "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    puts "\nCongratulations #{@user.name}! You're no longer bored! ... for now... \n"
  end
end

# method 1 to what_next_selections
def add_to_activities
  UserActivity.find_or_create_by(user_id: @user.id, activity_id: @activity.id)
end
...
def create_activity_from_api(option)
  ...
  @activity = Activity.find_or_create_by(name: activity_hash["activity"], accessibility: activity_hash["accessibility"], category: activity_hash["type"], participants: activity_hash["participants"], price: activity_hash["price"])
  puts ""
end
...
#     list_of_categories
#     category = gets.chomp.downcase
#     a = create_activity_from_api(category)
